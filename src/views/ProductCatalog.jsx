    ...
    <section className="weni-view-product-catalog">
      <section className="weni-view-product-catalog__products">
        {productGroups.map((productGroup, groupIndex) => (
          <section key={groupIndex} className="weni-product-group">
            <h2 className="weni-product-group__title">
              {productGroup.title}
            </h2>
            ...
            {productGroup.products.map((product, productIndex) => (
              <InlineProduct
                ...
                onClick={() => setCurrentPage({ view: 'product-details', title: 'Detalhes', props: { product } })}
              />
            ))}
          </section>
        ))}
      ...
      {totalItems > 0 && <footer className="weni-view-product-catalog__footer">
        <Button>Ver carrinho ({totalItems})</Button>
      </footer>}
    </section>
  );
}
